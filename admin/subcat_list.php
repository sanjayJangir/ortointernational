...
if (isset($_REQUEST['Delete'])) {
   if (is_array($_REQUEST['arr_ids'])) {
      ...
      function del_sub($CAT)
      {
         ...
         $ct_banner = mysqli_fetch_array(db_query("select category_inner_banner from tbl_category where category_id='$CAT'"));
         @unlink("../uploaded_files/$ct_banner[category_inner_banner]");
...
if (is_post_back()) {
   ...
   if (is_array($arr_ids)) {
      ...
      if (isset($_REQUEST['Activate']) || isset($_REQUEST['Activate_x'])) {
         ...
         db_query("update tbl_category set category_is_top='Yes' where category_id in ($str_ids)");
         $_SESSION["msg"] = "selected categories is set for top. ";
      } else if (isset($_REQUEST['remove_top']) || isset($_REQUEST['remove_top_x'])) {
         db_query("update tbl_category set category_is_top='No' where category_id in ($str_ids)");
         $_SESSION["msg"] = "selected categories is removed from top. ";
      }
   }
