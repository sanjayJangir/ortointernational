...
if (isset($_REQUEST['Delete'])) {
   if (is_array($_REQUEST['arr_ids'])) {
      ...
      $data = db_query("select * from tbl_attachments where id in ($str_ids)");
      while ($row = mysqli_fetch_array($data)) {
         $del_img = "../uploaded_files/" . $row['attachment_image_name'];
         @unlink($del_img);
         db_query("delete from tbl_attachments where id='$row[id]'");
      }
   }
   header("Location: manage-attachment.php?product_id=$product_id&subcatid=$subcatid&catid=$catid");
   exit;
}
...
if (is_post_back()) {
   ...
   if (is_array($arr_ids)) {
      ...
      if (isset($_REQUEST['Activate']) || isset($_REQUEST['Activate_x'])) {
         db_query("update tbl_attachments set category_status='Active' where id in ($str_ids)");
         $_SESSION["msg"] = "selected categories are activated. ";
      } else if (isset($_REQUEST['Deactivate']) || isset($_REQUEST['Deactivate_x'])) {
         db_query("update tbl_attachments set category_status='Inactive' where id in ($str_ids)");
         $_SESSION["msg"] = "selected categories are deactivated. ";
      } else if (isset($_REQUEST['set_home']) || isset($_REQUEST['set_home_x'])) {
         db_query("update tbl_attachments set category_for_home='Yes' where id in ($str_ids)");
         $_SESSION["msg"] = "selected categories is set for home. ";
      } else if (isset($_REQUEST['remove_home']) || isset($_REQUEST['remove_home_x'])) {
         db_query("update tbl_attachments set category_for_home='No' where id in ($str_ids)");
...
$sql = "select * from  tbl_attachments where 1 and product_id='$product_id'";
...
<div class="content-wrapper">
   ...
   <section class="content-header">
      ...
      <div class="header-title">
         <h1>Attachment</h1>
         <small>Attachment List</small>


         <a href="addedit-attachment.php?id=<?= 0 ?>&product_id=<?= $product_id ?>&subcatid=<?= $subcatid ?>&catid=<?= $catid ?>"><button id="btn-go-back" type="button" class="btn btn-labeled btn-inverse m-b-5 pull-right">
    ...
    border: solid thin #337ab785;"><i class="fa fa-chevron-circle-left" aria-hidden="true"></i>
         ...
         <span class="count-num">Total : <?= db_scalar("SELECT COUNT(id) FROM tbl_attachments WHERE 1 AND product_id='$product_id'") ?></span>
                                    ...
                                    <tr>
                                       ...
                                       <td align="center">
                                          ...
                                          <?php if ($attachment_image_name != "") { ?>
                                             <img src="../uploaded_files/<?= $attachment_image_name ?>" class="thumbnail" style="width:65px;height:65px; margin-top:0;margin-bottom:0" />
                                          <?php } else { ?>
                                       ...
                                       <td class="text-center v-middle">
                                          <a href="addedit-attachment.php?id=<?= $line_raw["id"] ?>&product_id=<?= $product_id ?>&subcatid=<?= $subcatid ?>&catid=<?= $catid ?>"><button type="button" class="btn btn-add btn-sm"><i class="fa fa-pencil"></i></button>
                                          </a>
                                       </td>
