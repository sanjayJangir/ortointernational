...
if (is_post_back()) {
   ...
   if (is_array($arr_ids)) {
      ...
      if (isset($_REQUEST['Delete'])) {
         ...
         db_query("DELETE FROM tbl_appointment WHERE enquiry_id in ($str_ids)");
...
$sql = "select * from  tbl_appointment   where 1 ";
...
<div class="content-wrapper">
   ...
   <section class="content-header">
      ...
      <div class="header-title">
         ...
         <span class="count-num">Total : <?= db_scalar("SELECT COUNT(enquiry_id) FROM  tbl_appointment ") ?></span>
                              ...
                              <tbody>
                                 ...
                                 ?>
                                    <tr>
                                       ...
                                       <td class=" v-middle">
                                          <p><a href="#" target="_blank"><i class="fa fa-search-plus fa-lg pull-left" style="color:#000"></i>&nbsp; <?= $line_raw["enquiry_name"] ?> </a></p>
                                       </td>
                                       <td class="text-center v-middle">
                                          <a href="tel:<?= $line_raw["enquiry_comp_name"] ?>"><?= $line_raw["enquiry_comp_name"] ?></a>
                                       </td>
                                       ...
                                       <td class="text-center v-middle">
                                          ...
                                          <?php if ($line_raw['enquiry_source'] != "") { ?>
                                             <div class="col-lg-12 bold source" align="center">
                                                <h5 class="source-title">Citizenship: <?= $line_raw['enquiry_source'] ?> </h5>
                                                <?php
                                                if (!empty($line_raw['enquiry_subject'])) { ?>
                                                   <h5 class="source-title">Maritial Status: <?= $line_raw['enquiry_subject'] ?> </h5>
                                                <?php } ?>
                                       ...
                                       <td class="text-center v-middle">
                                          <a href="tel:<?= $line_raw["enquiry_detail"] ?>"><?= $line_raw["enquiry_detail"] ?></a>
                                       </td>
                                       <td class="text-center v-middle">
                                          <a href="tel:<?= $line_raw["enquiry_address"] ?>"><?= $line_raw["enquiry_address"] ?></a>
                                       </td>
                                       <td class="text-center v-middle">
                                          <a href="tel:<?= $line_raw["Visa"] ?>"><?= $line_raw["Visa"] ?></a>
                                       </td>
                                       <td class="text-center v-middle">
