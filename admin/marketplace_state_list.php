...
if ($_REQUEST['st'] != "") {
   ...
   if ($st == 1) {
      $sql = "UPDATE tbl_marketplace SET category_status='Inactive' WHERE category_id='$catID' ";
      $res = db_query($sql);
      if ($res > 0) {
         $_SESSION["msg"] = "selected state is deactivated successfully.";
      }
   } else {
      $sql = "UPDATE tbl_marketplace SET category_status='Active' WHERE category_id='$catID' ";
      $res = db_query($sql);
      if ($res > 0) {
   ...
   header("location:marketplace_state_list.php");
   exit;
}
...
if (is_post_back()) {
   ...
   if (is_array($arr_ids)) {
      ...
      if (isset($_REQUEST['Activate']) || isset($_REQUEST['Activate_x'])) {
         ...
         $_SESSION["msg"] = "selected states is set for home. ";
         ...
         $_SESSION["msg"] = "selected states is removed from home. ";
      } else if (isset($_REQUEST['Delete']) || isset($_REQUEST['Delete_x'])) {
         db_query("delete from tbl_marketplace where category_parent_id in ($str_ids)");
         db_query("delete from tbl_marketplace where category_id in ($str_ids)");
         $_SESSION["msg"] = "selected states is deleted. ";
      }
   }
   if (isset($_REQUEST['Update'])) {
      ...
      $_SESSION["msg"] = "Selected states order is set.";
   }
...
$sql = "select * from  tbl_marketplace   where 1 and category_parent_id='0'";
...
<div class="content-wrapper">
   ...
   <section class="content-header">
      ...
      <div class="header-title">
         ...
         <a href="addedit-marketplace-state.php?id=0"><button id="btn-go-back" type="button" class="btn btn-labeled btn-inverse m-b-5 pull-right">
               <span class="btn-label"><i class="fa fa-plus" aria-hidden="true"></i>
               </span>Add Market Place
            </button></a>

         <span class="count-num">Total : <?= db_scalar("SELECT COUNT(category_id) FROM tbl_marketplace WHERE 1 AND category_parent_id='0'") ?></span>
                              ...
                              <tbody>
                                 ...
                                 ?>
                                    <tr>
                                       ...
                                       <td class="text-center v-middle"><a href="marketplace_district_list.php?catID=<?= $line_raw["category_id"] ?>"><?= $line_raw["category_name"] ?> <i class="fa fa-arrow-right"></i></a>
                                       ...
                                       <td class="text-center v-middle">
                                          ...
                                          <a href="addedit-marketplace-district.php?id=0&subCatID=<?= $line_raw["category_id"] ?>"><button type="button" class="btn btn-info btn-outline btn-rounded w-md m-b-5"><i class="fa fa-plus">&nbsp;&nbsp;</i></span>District</button></a>
                                       ...
                                       <td class="text-center v-middle">
                                          <?php if ($line_raw["category_status"] == "Active") { ?>
                                             <a href="marketplace_state_list.php?st=1&pid=<?= $line_raw["category_id"] ?>"><span class="label-custom label label-default">Active</span></a>
                                          <?php } else { ?>
                                             <a href="marketplace_state_list.php?st=0&pid=<?= $line_raw["category_id"] ?>"><span class="label-danger label label-default">Inactive</span></a>
                                          <?php } ?>
                                       ...
                                       <td class="text-center v-middle">
                                          <a href="addedit-marketplace-state.php?id=<?= $line_raw["category_id"] ?>"><button type="button" class="btn btn-add btn-sm"><i class="fa fa-pencil"></i></button>
                                          </a>
                                       </td>
