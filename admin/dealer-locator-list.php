...
if ($_REQUEST['st'] != "") {
    ...
    if ($st == 1) {
        $sql = "UPDATE tbl_dealers SET category_status='Inactive' WHERE category_id='$catID' ";
        $res = db_query($sql);
        if ($res > 0) {
            $_SESSION["msg"] = "selected location is deactivated successfully.";
        }
    } else {
        $sql = "UPDATE tbl_dealers SET category_status='Active' WHERE category_id='$catID' ";
        $res = db_query($sql);
        if ($res > 0) {
    ...
    header("location:dealer-locator-list.php");
    exit;
}
...
if (is_post_back()) {
    ...
    if (is_array($arr_ids)) {
        ...
        if (isset($_REQUEST['Activate']) || isset($_REQUEST['Activate_x'])) {
            ...
            $data = db_query("select * from tbl_dealers where category_parent_id in ($str_ids)");
            while ($row = mysqli_fetch_array($data)) {
                @unlink("../dealer/$row[category_image_name]");
                db_query("delete from tbl_dealers where category_id='$row[category_id]'");
            }

            db_query("delete from tbl_dealers where category_id in ($str_ids)");
            $_SESSION["msg"] = "selected locations is deleted. ";
        }
    }
    if (isset($_REQUEST['Update'])) {
        ...
        $_SESSION["msg"] = "selected locations order is set.";
    }
...
$sql = "select * from  tbl_dealers   where 1 and category_parent_id='0'";
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <div class="header-title">
            ...
            <small>Dealer Locator List</small>

            <?php
            $count = db_scalar("select COUNT(category_id) from tbl_dealers where category_parent_id=0");
            if ($count < 2) { ?>
                <a href="addedit-dealer-locator.php?id=0"><button id="btn-go-back" type="button" class="btn btn-labeled btn-inverse m-b-5 pull-right">
                        <span class="btn-label"><i class="fa fa-plus" aria-hidden="true"></i>
                        </span>Add Dealer Locator
                    </button></a>

            <?php } ?>

            <span class="count-num">Total : <?= db_scalar("SELECT COUNT(category_id) FROM tbl_dealers WHERE 1 AND category_parent_id='0'") ?></span>
                                        ...
                                        <tbody>
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    <td class="text-center v-middle"><a href="dealer-locator-info-list.php?catID=<?= $line_raw["category_id"] ?>"><?= $line_raw["location_name"] ?> <i class="fa fa-arrow-right"></i></a>
                                                    ...
                                                    <td class="text-center v-middle">
                                                        ...
                                                        <a href="addedit-dealer-locator-info.php?id=0&subCatID=<?= $line_raw["category_id"] ?>"><button type="button" class="btn btn-info btn-outline btn-rounded w-md m-b-5"><i class="fa fa-plus">&nbsp;&nbsp;</i></span>Dealers</button></a>
                                                    ...
                                                    <td class="text-center v-middle">
                                                        <?php if ($line_raw["category_status"] == "Active") { ?>
                                                            <a href="dealer-locator-list.php?st=1&pid=<?= $line_raw["category_id"] ?>"><span class="label-custom label label-default">Active</span></a>
                                                        <?php } else { ?>
                                                            <a href="dealer-locator-list.php?st=0&pid=<?= $line_raw["category_id"] ?>"><span class="label-danger label label-default">Inactive</span></a>
                                                        <?php } ?>
                                                    ...
                                                    <td class="text-center v-middle">
                                                        <a href="addedit-dealer-locator.php?id=<?= $line_raw["category_id"] ?>"><button type="button" class="btn btn-add btn-sm"><i class="fa fa-pencil"></i></button>
                                                        </a>
                                                    </td>
