...
$isDel=db_query("UPDATE tbl_features SET feature_image_name='' WHERE 1 and id = '$id'");
@unlink("../uploaded_files/$delImage");
header("location:addedit-feature.php?id=$id&product_id=$product_id&subcatid=$subcatid&catid=$catid");
}
...
$imgToDel=db_scalar("SELECT feature_image_name FROM tbl_features WHERE  id='$id'");	
...
$sql = "update tbl_features set  
product_id='$product_id',      
name='$name',     
feature_image_name='$imgNAME',
...
$sql = "insert into tbl_features set 
product_id='$product_id',
name='$name',			
feature_image_name='$imgNAME',
description='$description', 	
...
if($id!=0) {
$result = db_query("select * from tbl_features where id = '$id'");
...
<small>Add/Edit Feature content</small>


<a href="manage-feature.php?product_id=<?=$product_id?>&subcatid=<?=$subcatid?>&catid=<?=$catid?>"><button id="btn-go-back" type="button" class="btn btn-labeled btn-inverse m-b-5 pull-right">
...
<img src="../uploaded_files/<?=$feature_image_name?>" class="form-control" style="width:150px;height:150px" />
<?php }else{?>
<img src="assets/dist/img/no-image.jpg" class="form-control" style="width:120px;height:150px" />
<?php }?>

<?php if($feature_image_name!=""){ ?>
<div class="col-lg-12 " ><a href="addedit-feature.php?delImage=<?=$feature_image_name?>&id=<?=$id?>&product_id=<?=$product_id?>&subcatid=<?=$subcatid?>&catid=<?=$catid?>" style="font-weight:600;margin-left:15px;text-decoration:underline" >Remove Image</a>                  
</div>
<?php }?>
...
<div class="form-group col-lg-12">
<label>Feature Meta Title</label>
<textarea class="form-control" rows="3" name="meta_title" id="meta_title" placeholder="Enter Feature meta title here" ><?=$meta_title?></textarea>
</div>
<div class="form-group col-lg-12">
<label>Feature Meta Description</label>
<textarea class="form-control" rows="3" placeholder="Enter Feature meta description here" name="meta_description" id="meta_description"><?=$meta_description?></textarea>
</div>
<div class="form-group col-lg-12">
<label>Feature Meta Keyword</label>
<textarea class="form-control" rows="3" name="meta_keyword" placeholder="Enter Feature meta keywords here" id="meta_keyword"><?=$meta_keyword?></textarea>
</div>
