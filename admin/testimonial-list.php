...
if ($_REQUEST['st'] != "") {
   ...
   if ($st == 1) {
      $sql = "UPDATE tbl_testimonial SET test_status='Inactive' WHERE id='$catID' ";
      $res = db_query($sql);
      if ($res > 0) {
         $_SESSION["msg"] = "Selected testimonial is deactivated successfully.";
      }
   } else {
      $sql = "UPDATE tbl_testimonial SET test_status='Active' WHERE id='$catID' ";
      $res = db_query($sql);
      if ($res > 0) {
...
if (is_post_back()) {
   ...
   if (is_array($arr_ids)) {
      ...
      if (isset($_REQUEST['Activate']) || isset($_REQUEST['Activate_x'])) {
         db_query("update tbl_testimonial set test_status='Active' where id in ($str_ids)");
         $_SESSION["msg"] = "selected testimonials are activated. ";
      } else if (isset($_REQUEST['Deactivate']) || isset($_REQUEST['Deactivate_x'])) {
         db_query("update tbl_testimonial set test_status='Inactive' where id in ($str_ids)");
         $_SESSION["msg"] = "selected testimonials are deactivated. ";
      } else  if (isset($_REQUEST['Delete'])) {
         $count = COUNT($arr_ids);
         for ($i = 0; $i < $count; $i++) {
            $old_img = db_scalar("select test_image_name from tbl_testimonial where id='$arr_ids[$i]'");
            @unlink("../test_images/$old_img");
         }

         db_query("DELETE FROM tbl_testimonial WHERE id in ($str_ids)");
         $_SESSION["msg"] = "selected testimonials are deleted. ";
      }
   }
   if (isset($_REQUEST['Update'])) {
      foreach ($_REQUEST['test_order_by'] as $key => $value) {
         db_query("update tbl_testimonial set test_order_by='$value' where id='$key'");
      }
...
<div class="content-wrapper">
   ...
   <section class="content-header">
      ...
      <div class="header-title">
         ...
         <span class="count-num">Total : <?php ?></span>
                                    ...
                                    <tr>
                                       ...
                                       <td class="text-center v-middle">
                                          <?php if ($line_raw["test_status"] == "Active") { ?>
                                             <a href="testimonial-list.php?st=1&pid=<?= $line_raw["id"] ?>"><span class="label-custom label label-default">Active</span></a>
                                          <?php } else { ?>
                                             <a href="testimonial-list.php?st=0&pid=<?= $line_raw["id"] ?>"><span class="label-danger label label-default">Inactive</span></a>
                                          <?php } ?>
                                       ...
                                       <td class="v-middle" align="center">
                                          <input type="text" name="test_order_by[<?= $id ?>]" id="test_order_by[<?= $id ?>]" value="<?= $id ?>" class="form-control" style="width:40px" />
                                       </td>
                                       ...
                                       <td class="text-center v-middle">
                                          ...
                                          <a href="add-edit-testimonial.php?id=<?php echo $line_raw["id"]; ?>"><button type="button" class="btn btn-add btn-sm"><i class="fa fa-pencil"></i></button>
                                          </a>
                                       </td>
