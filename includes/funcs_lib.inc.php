...
$checkstr .= "<table $tableattr>
    ...
    <tr>\n";
        ...
        $checkstr .= "<td width='" . $colwidth . "%' $style valign=top><INPUT TYPE='radio' $javascript NAME='$name'
                value='$key' $tochecked> $value </td>\n";
        $j++;
        }
        }
        }
        $j--;
        for ($x = $j % $cols; $x < 4; $x++) { if ($x !=3) { $checkstr .="<td>&nbsp;</td>\n" ; } else { $checkstr
            .="<td>&nbsp;</td></tr>\n" ; } } $checkstr .="</table>" ; return $checkstr; } function show_thumb($file_org,
            $width, $height, $ratio_type='width_height' ) { if (LOCAL_MODE) { $method='gd' ; } else { $method='im' ; }
            return midas_thumb::show_thumb($file_org, $width, $height, $ratio_type, $method, 'php' ); } function
            ms_parse_keywords($keywords) { $arr_keywords=array(); $dq_end=true; $sp_end=true; for ($i=0; $i <
            strlen($keywords); $i++) { $cur_token=$keywords[$i]; if ($cur_token=='"' ) { if ($dq_start) { $dq_end=true;
            $dq_start=false; $arr_keywords[]=$cur_keyword; $cur_keyword='' ; } else if ($dq_end) { $dq_end=false;
            $dq_start=true; $sp_start=false; } else { $dq_end=false; $dq_start=true; } } else if ($cur_token==' ' ) { if
            ($sp_start || $dq_end) { $sp_end=true; $sp_start=false; $arr_keywords[]=$cur_keyword; $cur_keyword='' ; }
            else if ($sp_end && !$dq_start) { $sp_end=false; $sp_start=true; } else if ($dq_start) { $cur_keyword
            .=$cur_token; } } else { $cur_keyword .=$cur_token; } } $arr_keywords[]=$cur_keyword; return $arr_keywords;
            } function pagesize_dropdown($name, $value) { $arr=array('10'=> '10', '25' => '25', '50' => '50', '100' =>
            '100', '250' => '250');
            $m = $_GET;
            unset($m['pagesize']);
            return array_dropdown($arr, $value, $name, ' onchange="location.href=\'' . $_SERVER['PHP_SELF'] .
            qry_str($m) . '&pagesize=\'+this.value" ');
            }
            ...
            function enum_dropdown($table, $column, $name, $sel_value = '', $extra = '', $choose_one = '', $arr_skip =
            array())
            {
            ...
            switch ($type) {
            case 'textfield':
            $add_query = '<input name="' . $name . '" type="text" id="' . $name . '"
                value="' . $sel_value . '" ' . $extra . ' class="textfield">';
            break;
            case 'password':
            $add_query = '<input name="' . $name . '" type="password" id="' . $name . '"
                value="' . $sel_value . '" ' . $extra . ' class="textfield">';
            break;
            case 'textarea':
            $add_query = '<textarea name="' . $name . '" id="' . $name . '" rows="5" cols="50" ' . $extra . '
                class="textfield">' . $sel_value . '</textarea>';
            break;
            case 'select':
            ...
            $add_query = '<select name="' . $name . '" ' . $extra . '>';
                ...
                $add_query .= '<option value="' . $key . '" ';
					if ($sel_value == $key) {
						$add_query .= ' selected'; } $add_query .=' >' . $value . "</option>" ; $add_query .="\r\n" ; } $add_query
                    .='</select>' ; } break; case 'list' : if (is_array($arr_values)) { $add_query='<select name="' .
                    $name . '" size ="4" multiple  ' . $extra . '>' ; foreach ($arr_values as $key=> $value) {
                    $add_query .= '
                <option value="' . $key . '" ';
					if (in_array($key, $sel_value)) {
						$add_query .= ' selected'; } $add_query .=$value . "</option>" ; $add_query .="\r\n" ; } $add_query .='</select>'
                    ; } break; case 'radio' : if (is_array($arr_values)) { foreach ($arr_values as $key=> $value) {
                    $add_query .= '<input type="radio" name="' . $name . '" value="' . $key . '" ';
					if ($sel_value == $key) {
						$add_query .= ' checked'; } $add_query .=$extra . '> ' . $value; } } break; case 'checkbox' : if
                        (is_array($arr_values)) { foreach ($arr_values as $key=> $value) {
                    $add_query .= '<input type="checkbox" name="arr_' . $name . '[]" value="' . $key . '"';
					if (in_array($key, $sel_value)) {
						$add_query .= ' checked'; } $add_query .=$extra . '> ' . $value; } } break; case 'hidden' :
                        $add_query='<input name="' . $name . '" type="hidden" id="' . $name . '" value="' . $sel_value
                        . '" ' . $extra . '>' ; break; case 'function' : $function=str_replace('{1}', $name, $values);
                        $function=str_replace('{v}', $extra, $function); $add_query=eval('echo ' . $function);
			break;
	}
	return $add_query;
}
function file_size_format($file_size)
{
	if ($file_size > 1024 * 1024 * 1024) {
		return round($file_size / (1024 * 1024 * 1024), 2) . ' GB'; } else if ($file_size> 1024 * 1024) {
                    ...
                    function recursive_dropdown($id_column, $name_column, $parent_id_column, $order_column, $table,
                    $where, $name, $sel_value, $skip, $extra = '', $choose_one = '', $parent_id = 0, $level = 0)
                    {
                    $level++;
                    $sql = "select $id_column, $name_column from $table where $parent_id_column='$parent_id' and
                    $id_column!='$skip'";
                    if ($where != '') {
                    $sql .= " and $where ";
                    }
		...
		$add_query_dropdown .= $level == 1 ? ' </select>' : '';
                            ...
                            $array_trans = array('<'=> "&lt;", '>' => "&gt;", '"' => "&quot;", "'" => "&#039;");
                                ...
                                //trigger_error("<div style='color:red;'>Cannot load plugin: $name</div>",
                                E_USER_ERROR);
                                }
                                }
